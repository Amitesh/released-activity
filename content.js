	...
	var Activity = {
		...
		getSprint: function(){
			...
			if(!sprint){
				...
				console.error(err);
				// alert(err);
				Activity.sendMessage('danger', err);
				return;
			}
		...
		collect: function(){
			...
			if(nextPageBtn){
				...
				Activity.sendMessage('info', 'Loading next page. Please wait... ');
				setTimeout(Activity.collect, pageLoadDelay);
			}else{
				// Send the data to popup through messaging
				console.log('==== done ====');
				console.log(JSON.stringify(sprintWise));
				Activity.updateList(sprintWise);
			}
		},

		updateList: function(sprintWise){
			jQuery.getJSON(chrome.extension.getURL('/js/released-activities.json'), function(data){
  			var updatedData = {};
  			var data = _.merge({}, sprintWise, data)
  			_.keys(data).sort().forEach(function(k){ updatedData[k] = data[k]; });

  			console.log('==== Final ====', JSON.stringify(updatedData));
  			Activity.send(JSON.stringify(updatedData));
  			// alert('Done');
  			Activity.sendMessage('success', 'Released activities populated.');
  		});
		},
		...
		sendMessage: function(type, message){
			//Broadcast message to all listener (popup.js and background.js)
			chrome.runtime.sendMessage({action: 'popup.alert', alertType: type, from: 'content', message: message}, function(response){
				console.log(response.from, ':', response.message);
			});
		}
	};
